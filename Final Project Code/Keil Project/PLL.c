...
void PLL_Init(void){ uint32_t timeout;
  ...
  //    fVCO = 480,000,000 Hz (arbitrary, but presumably as small as needed)|||CHANGED TO 420,000,000 chosen for easy division for 14Mhz bus
...
#define MINT        84  // 420,000,000  = (25,000,000/((0 + 1)*(4 + 1)))*   (84 + (0/1,024)) ||| 480,000,000 = (25,000,000/((0 + 1)*(4 + 1)))  *(96 + (0/1,024))
#define MFRAC        0  // zero to reduce jitter
  //    SysClk = fVCO / (PSYSDIV + 1) = 420,000,000 / 30 = 14,000,000 ||| PSYDIV = 29 also the bus freq/speed
...
/*
PSYSDIV  SysClk (Hz) |||||| TABLE NOT RELEVANT BECAUSE FVSCO IS NOW 420M not 480 and PSYSDIV = 29 is for 14MHz SysClk
  3     120,000,000
  4      96,000,000
  5      80,000,000
  7      60,000,000
