...
void SysTick_Wait10ms(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
		//1/14,000,000 = 71.4 ns for one cycle, Cycles for 10ms is 10,000,000ns / 71.4ns = 140000
    SysTick_Wait(140000);  // wait 10ms (assumes 120 MHz clock) || NOW ASSUMES 14MHz clock 140000
  }
}
...
void SysTick_Wait10us(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
    SysTick_Wait(1400);  // change this to wait 10us (assumes 120 MHz clock)
  }
}
